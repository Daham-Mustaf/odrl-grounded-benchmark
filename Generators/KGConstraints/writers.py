...
def write_problem(p: dict, out_dir: Path, policies_dir: Path,
                  reports_dir: Path) -> list[Path]:
    """Write both queries, the policy and the expected report."""
    written = []
    if not p.get("ungrounded"):
        for q in (1, 2):
            written.append(write_fof(p, q, out_dir))
            written.append(write_smt2(p, q, out_dir))
    written.append(write_ttl_policy(p, policies_dir))
    written.append(write_report(p, expected_verdict(p), reports_dir))
    return written
